...
int BotAI_GetEntityState( int entityNum, entityState_t *state ) {
	...
	if (!ent->r.linked) return qfalse;
        if ( !(g_gametype.integer == GT_ELIMINATION || g_gametype.integer == GT_LMS ||g_instantgib.integer || g_rockets.integer || g_weaponArena.integer || g_elimination_allgametypes.integer || g_gametype.integer==GT_CTF_ELIMINATION)
