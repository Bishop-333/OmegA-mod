...
int BotAI_GetEntityState( int entityNum, entityState_t *state ) {
	...
	if ( !( ( ( G_IsElimGametype() || g_elimination_allgametypes.integer ) && !g_elimination_items.integer ) || g_instantgib.integer || g_weaponArena.integer ) && ( ent->r.svFlags & SVF_NOCLIENT ) ) return qfalse;
...
int BotAIStartFrame( int time ) {
	...
	if ( botlib_residual >= thinktime ) {
		...
		for ( i = 0; i < level.num_entities; i++, s++, ent++ ) {
			...
			if ( !( ( ( G_IsElimGametype() || g_elimination_allgametypes.integer ) && !g_elimination_items.integer ) || g_instantgib.integer || g_weaponArena.integer ) && ent->r.svFlags & SVF_NOCLIENT ) {
				if ( *s == qfalse ) {
					*s = qtrue;
					trap_BotLibUpdateEntity( i, NULL );
				}
