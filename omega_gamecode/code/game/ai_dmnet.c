...
/*
==================
BotLTG_TeamAccompany
==================
*/
static int BotLTG_TeamAccompany( bot_state_t *bs, bot_goal_t *goal ) {
...
==================
*/
static int BotLTG_CTF( bot_state_t *bs, bot_goal_t *goal ) {
...
static int BotGetLongTermGoal( bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal ) {
	...
	if ( gametype == GT_CTF || gametype == GT_CTF_ELIMINATION ) {
		if ( BotLTG_CTF( bs, goal ) ) {
			return qtrue;
		}
	}
#endif //CTF
	else if ( gametype == GT_1FCTF ) {
		if ( BotLTG_1FCTF( bs, tfl, goal ) ) {
			return qtrue;
		}
	} else if ( gametype == GT_OBELISK ) {
		if ( BotLTG_Obelisk( bs, tfl, goal ) ) {
			return qtrue;
		}
	} else if ( gametype == GT_HARVESTER ) {
		if ( BotLTG_Harvester( bs, goal ) ) {
			return qtrue;
		}
	}
	//normal goal stuff
