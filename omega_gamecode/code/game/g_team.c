...
================
*/
static int getDomPointNumber( gentity_t *point ) {
	int i;
...
================
*/
static void Team_Dom_TakePoint( gentity_t *point, int team, int clientnumber ) {
	gitem_t *it;
	vec3_t origin;
	int i;
...
================
*/
static void Team_DD_RemovePointAgfx( void ) {
	if ( ddA != NULL ) {
		G_FreeEntity( ddA );
		ddA = NULL;
	}
...
================
*/
static void Team_DD_RemovePointBgfx( void ) {
	if ( ddB != NULL ) {
		G_FreeEntity( ddB );
		ddB = NULL;
	}
...
================
*/
static void Team_DD_makeA2team( gentity_t *target, int team ) {
	gitem_t *it;
	Team_DD_RemovePointAgfx();
	it = NULL;
	if ( team == TEAM_NONE )
...
================
*/
static void updateDDpoints( void ) {
...
static int Team_TouchDoubleDominationPoint( gentity_t *ent, gentity_t *other, int team ) {
...
static int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	// add the sprite over the player's head
	other->client->ps.eFlags &= ~( EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST | EF_AWARD_DEFEND | EF_AWARD_CAP | EF_AWARD_HEADSHOT );
	...
	for ( i = 0; i < g_maxclients.integer; i++ ) {
		...
		if ( player->client->sess.sessionTeam !=
			...
			if ( player->client->pers.teamState.lastreturnedflag +
				...
				// add the sprite over the player's head
				player->client->ps.eFlags &= ~( EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST | EF_AWARD_DEFEND | EF_AWARD_CAP | EF_AWARD_HEADSHOT );
...
*/
#define MAX_TEAM_SPAWN_POINTS 32
static gentity_t *SelectRandomDDSpawnPoint( void ) {
	gentity_t *spot;
	int count;
	int selection;
	gentity_t *spots[MAX_TEAM_SPAWN_POINTS];
...
/*
================
SelectRandomTeamDDSpawnPoint
================
*/
static gentity_t *SelectRandomTeamDDSpawnPoint( team_t team ) {
	gentity_t *spot;
	int count;
	int selection;
	gentity_t *spots[MAX_TEAM_SPAWN_POINTS];
