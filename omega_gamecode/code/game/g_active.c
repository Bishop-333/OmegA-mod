...
void G_FrozenPlayerDamage( gentity_t *targPlayer, gentity_t *targ, gentity_t *attacker,
	...
	if ( mod == MOD_SUICIDE || mod == MOD_FALLING || mod == MOD_LAVA || mod == MOD_SLIME || mod == MOD_TRIGGER_HURT || mod == MOD_CRUSH || mod == MOD_UNKNOWN || mod == MOD_WATER || mod == MOD_TARGET_LASER || mod == MOD_TELEFRAG || mod == MOD_JUICED ) {
		...
		if ( targPlayer->frozenPlayer && targPlayer->frozenPlayer->frozenPlayer && targPlayer->frozenPlayer->frozenPlayer == targPlayer && targPlayer->frozenPlayer->die ) {
...
void G_ClientAcceleratedThaw( gentity_t *player ) {
	...
	if ( client->sess.sessionTeam == TEAM_FREE || !player->frozenPlayer || !player->frozenPlayer->frozenPlayer || player->frozenPlayer->frozenPlayer != player ) {
		return;
	}
	...
	for ( i = 0; i < 3; ++i ) {
		mins[i] = frozenRemnant->r.currentOrigin[i] - FREEZETAG_THAWRADIUS;
		maxs[i] = frozenRemnant->r.currentOrigin[i] + FREEZETAG_THAWRADIUS;
	}
	...
	for ( e = 0; e < numListedEntities; e++ ) {
		...
		if ( !te->client || te->client->sess.sessionTeam != client->sess.sessionTeam || te->client->ps.pm_type == PM_DEAD || te == player ) {
			continue;
		}
		...
		if ( client->freezetag_thawedBy == -1 ||
	...
	if ( friend == -1 ) {
		...
		client->freezetag_thawed = 0.0;
		frozenRemnant->s.loopSound = 0;
		return;
	}
	frozenRemnant->s.loopSound = G_FreezeThawSound();
	client->freezetag_thawedBy = friend;
	if ( client->sess.spectatorState == SPECTATOR_FOLLOW && level.time - client->lastSpecatorSwitchTime > 1000 ) {
...
void G_ClientSetFrozenState( gentity_t *ent ) {
	...
	if ( ent->client->freezetag_thawedBy == -1 ) {
		progress = ent->client->freezetag_autoThawed;
	} else {
		progress = ent->client->freezetag_thawed;
	}

	// we pack all these things into 1 field because we're running out of fields in  EF_* and PMF_*
	ent->client->ps.stats[STAT_FROZENSTATE] = (unsigned int)( 0xff * progress ) << 2;
	...
	if ( ent->frozenPlayer && ent->frozenPlayer->frozenPlayer && ent->frozenPlayer->frozenPlayer == ent ) {
		...
		ent->frozenPlayer->s.modelindex2 = (unsigned int)( 0x7f * progress ) << 1;
		// to make the ice shell look like it's melting
		ent->frozenPlayer->s.modelindex2 |= ( ent->client->ps.stats[STAT_FROZENSTATE] & FROZENSTATE_THAWING ) ? 1 : 0;
	}
}
...
void G_ClientThawNow( gentity_t *ent, int thawedBy ) {
	...
	ent->client->freezetag_autoThawed = 1.0;
	...
	if ( thawedBy >= 0 && thawedBy < MAX_CLIENTS ) {
		...
		if ( thawingEnt->client->pers.connected == CON_CONNECTED ) {
			trap_SendServerCommand( ent - g_entities, va( "cpm \"Thawed by %s\"", thawingEnt->client->pers.netname ) );
			// adding score for thawing makes no sense for GT_TEAM
			if ( g_gametype.integer > GT_TEAM && g_ffa_gt != 1 ) {
				AddScore( thawingEnt, ent->r.currentOrigin, 1 );
			}
			thawingEnt->flags |= FL_FORCE_GESTURE;
		}
	}
}
...
void G_ClientThaw( gentity_t *ent ) {
	...
	if ( client->freezetag_autoThawed >= 1.0 ) {
		client->freezetag_autoThawed = 1.0;
	}
	G_ClientSetFrozenState( ent );

	if ( client->freezetag_thawed >= 1.0 ) {
		G_ClientThawNow( ent, ent->client->freezetag_thawedBy );
	} else if ( client->freezetag_autoThawed >= 1.0 ) {
		G_ClientThawNow( ent, -1 );
	}
}
...
void G_TouchTriggers( gentity_t *ent ) {
	...
	for ( i = 0; i < num; i++ ) {
		...
		if ( hit->s.eType == ET_ITEM ) {
			if ( !BG_PlayerTouchesItem( &ent->client->ps, &hit->s, level.time, g_easierPickup.integer ) ) {
				continue;
			}
		} else {
			if ( !trap_EntityContact( mins, maxs, hit ) ) {
...
static void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if ( G_IsElimTeamGametype() && client->sess.spectatorState != SPECTATOR_FOLLOW && g_elimination_lockspectator.integer > 1 && ent->client->sess.sessionTeam != TEAM_SPECTATOR ) {
...
static void ClientThink_real( gentity_t *ent ) {
	...
	if ( client->noclip ) {
		...
		if ( !( ent->r.svFlags & SVF_BOT ) && g_spectateOnDeath.integer && level.time - client->deathTime > 1700 ) {
	...
	pm.ps = &client->ps;
	pm.cmd = *ucmd;
	if ( ( pm.ps->pm_type == PM_DEAD && ent->client->frozen != FROZEN_ONMAP ) || ( g_spectateOnDeath.integer && pm.ps->pm_type == PM_SPECTATOR ) ) {
...
void ClientEndFrame( gentity_t *ent ) {
	...
	if ( g_freeze.integer ) {
		G_ClientThaw( ent );
		if ( ent->client->frozen == FROZEN_ONMAP && !ent->client->isEliminated ) {
			trap_UnlinkEntity( ent );
			return;
		}
	}
