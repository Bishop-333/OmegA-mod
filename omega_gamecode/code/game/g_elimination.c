...
void CheckLMS( void ) {
	...
	level.activeWarmup = g_elimination_activewarmup.integer;
	...
	if ( g_gametype.integer == GT_LMS ) {
		...
		if ( level.activeWarmup < 2 )
			level.activeWarmup = 2;                                //We need at least 2 seconds to spawn all players
		if ( level.activeWarmup >= g_elimination_warmup.integer )  //This must not be true
			g_elimination_warmup.integer = level.activeWarmup + 1; //Increase warmup

		//Force respawn
		if ( level.roundNumber != level.roundNumberStarted && level.time > level.roundStartTime - 1000 * level.activeWarmup && !level.roundRespawned ) {
...
void CheckElimination( void ) {
	...
	if ( G_IsElimTeamGametype() ) {
		...
		//This might be better placed another place:
		if ( level.activeWarmup < 1 )
			level.activeWarmup = 1;                                //We need at least 1 second to spawn all players
		if ( level.activeWarmup >= g_elimination_warmup.integer )  //This must not be true
			g_elimination_warmup.integer = level.activeWarmup + 1; //Increase warmup
