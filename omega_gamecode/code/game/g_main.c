...
vmCvar_t g_bluerespawntime;
vmCvar_t g_redrespawntime;
...
static cvarTable_t gameCvarTable[] = {
    ...
    { &g_blood, "com_blood", "2", 0, 0, qfalse },
...
\\7\\maps_harvester.cfg\\8\\maps_elimination.cfg\\9\\maps_ctf.cfg\\10\\maps_lms.cfg\\11\\maps_dd.cfg\\12\\maps_dom.cfg\\",
      CVAR_ARCHIVE | CVAR_NORESTART, 0, qfalse },
    ...
    { &g_juggernaut, "g_juggernaut", "1", CVAR_ARCHIVE, 0, qtrue },
...
========================
*/
static void SendAttackingTeamMessageToAllClients( void ) {
	int i;
...
=================
*/
static void StartLMSRound( void ) {
	int countsLiving;
...
=================
*/
static void RestartEliminationRound( void ) {
	DisableWeapons();
	level.roundNumberStarted = level.roundNumber - 1;
	level.roundStartTime = level.time + 1000 * g_elimination_warmup.integer;
	if ( !level.intermissiontime )
...
=================
*/
static void WarmupEliminationRound( void ) {
	EnableWeapons();
	level.roundNumberStarted = level.roundNumber - 1;
	level.roundStartTime = level.time + 1000 * g_elimination_warmup.integer;
	SendEliminationMessageToAllClients();
...
=============
*/
static void CheckDoubleDomination( void ) {
	if ( level.numPlayingClients < 1 ) {
		return;
	}

	if ( level.warmupTime != 0 ) {
		if ( ( ( level.pointStatusA == TEAM_BLUE && level.pointStatusB == TEAM_BLUE ) || ( level.pointStatusA == TEAM_RED && level.pointStatusB == TEAM_RED ) ) && level.timeTaken + 10 * 1000 <= level.time ) {
...
=============
*/
static void CheckDomination( void ) {
	int i;
	int scoreFactor = 1;
...
static void CheckTournament( void ) {
	...
	if ( g_gametype.integer == GT_TOURNAMENT ) {
		...
		if ( level.warmupTime < 0 ) {
			if ( level.numPlayingClients == 2 ) {
				if ( ( g_startWhenReady.integer && ( g_entities[level.sortedClients[0]].client->ready || ( g_entities[level.sortedClients[0]].r.svFlags & SVF_BOT ) ) && ( g_entities[level.sortedClients[1]].client->ready || ( g_entities[level.sortedClients[1]].r.svFlags & SVF_BOT ) ) ) || !g_startWhenReady.integer || !g_doWarmup.integer ) {
