...
void pushReward( sfxHandle_t sfx, qhandle_t shader, int rewardCount ) {
	...
	for ( i = 0; i < MAX_REWARDSTACK; ++i ) {
		if ( cg.rewardShader[i] == shader && ( cg.rewardTime[i] == -1 || ( cg.rewardTime[i] > 0 && cg.rewardTime[i] + CG_RewardTime( i ) > cg.time ) ) ) {
			found = qtrue;
			break;
		}
	}
...
static void CG_CheckLocalSounds( playerState_t *ps, playerState_t *ops ) {
	...
	// timelimit warnings
	if ( cgs.timelimit > 0 && cgs.timelimit <= 1000 && !cg.warmup ) {
		int msec;
