...
static void CG_SpecZooming( void ) {
	if ( cg.specZoomed ) {
		...
		if ( !( cg.snap->ps.stats[STAT_SPEC_ZOOM] & ZOOMING ) ) {
			cg.zoomed = qfalse;
			cg.specZoomed = qfalse;
			cg.zoomTime = cg.time;
		}
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	cg.renderingThirdPerson = ( ( cg_thirdPerson.integer && cgs.allowThirdperson ) || cg.snap->ps.stats[STAT_HEALTH] <= 0 || cgs.clientinfo[cg.snap->ps.clientNum].isProp || ( trap_Key_GetCatcher() & KEYCATCH_CONSOLE && cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR && !cg_paused.integer ) ) && cg.snap->ps.pm_type != PM_SPECTATOR;
