...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent, int team ) {
	...
	if ( ps || cent->currentState.clientNum == cg.predictedPlayerState.clientNum ) {
		if ( cg.predictedPlayerState.weapon == WP_RAILGUN && cg.predictedPlayerState.weaponstate == WEAPON_FIRING ) {
			...
			gun.shaderRGBA[0] = ci->color1[0] * 51 * ( 1.0 - f );
			gun.shaderRGBA[1] = ci->color1[1] * 51 * ( 1.0 - f );
...
int CG_GetWeaponSelect( void ) {
	return ( ( cg.snap->ps.pm_flags & PMF_FOLLOW ) || cg.demoPlayback ) ? cg.predictedPlayerState.weapon : cg.weaponSelect;
}
...
void CG_FireWeapon( centity_t *cent ) {
	...
	if ( ( cgs.gametype == GT_ELIMINATION || cgs.gametype == GT_CTF_ELIMINATION ) && cgs.roundStartTime >= cg.time && cg.snap->ps.pm_flags & PMF_ELIMWARMUP )
