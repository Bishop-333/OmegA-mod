...
static void CG_EntityEffects( const centity_t *cent ) {
	...
	// add loop sound
	if ( cent->currentState.loopSound && cg_ambient.integer ) {
...
/*
==================
CG_AddRefEntityForItem
==================
*/
static void CG_AddRefEntityForItem( centity_t *cent, refEntity_t *ent, const gitem_t *item, weaponInfo_t *wi, int itemIndex ) {
	int msec;
	float frac;
	...
	if ( !cg_simpleItems.integer ) {
		...
		if ( item->giType == IT_HEALTH || item->giType == IT_POWERUP ) {
			...
			if ( ( ent->hModel = cg_items[itemIndex].models[1] ) != 0 ) {
...
static void CG_Item( centity_t *cent ) {
	...
	CG_AddRefEntityForItem( cent, &ent, item, wi, es->modelindex );
}
...
void CG_Prop( centity_t *cent, clientInfo_t *ci ) {
	...
	for ( itemIndex = 1; itemIndex <= bg_numItems; itemIndex++ ) {
		if ( !Q_stricmp( ci->modelName, bg_itemlist[itemIndex].world_model[0] ) ) {
	...
	if ( cg_simpleItems.integer && item->giType != IT_TEAM ) {
		...
		VectorCopy( cent->lerpOrigin, ent.origin );
		ent.radius = 14;
		ent.customShader = cg_items[itemIndex].icon;
		ent.shaderRGBA[0] = 255;
		ent.shaderRGBA[1] = 255;
		ent.shaderRGBA[2] = 255;
		ent.shaderRGBA[3] = 255;
	...
	CG_AddRefEntityForItem( cent, &ent, item, wi, itemIndex );
}
...
static void CG_Missile( centity_t *cent ) {
	...
	CG_AddRefEntityWithPowerups( &ent, s1, TEAM_FREE, qtrue, cent->currentState.number );
}
...
void CG_AddPacketEntities( void ) {
	...
	for ( num = 0; num < cg.snap->numEntities; num++ ) {
		...
		//unlagged - early transitioning
		if ( !cg.nextSnap || ( cent->nextState.eType != ET_MISSILE && cent->nextState.eType != ET_GENERAL && cent->nextState.eType != ET_ITEM ) ) {
			//unlagged - early transitioning
			CG_AddCEntity( cent );
		} //Also unlagged
	}
