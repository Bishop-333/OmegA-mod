...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	...
	if ( CG_FindClientHeadFile( filename, sizeof( filename ), ci, teamName, headName, headSkinName, "icon", "skin" ) ) {
		...
		ci->modelIcon = trap_R_RegisterShaderNoMip( filename );
	} else if ( CG_FindClientHeadFile( filename, sizeof( filename ), ci, teamName, headName, headSkinName, "iconh", "tga" ) ) {
		ci->modelIcon = trap_R_RegisterShaderNoMip( filename );
	}
...
static qboolean CG_ScanForExistingClientInfo( clientInfo_t *ci ) {
	...
	for ( i = 0; i < cgs.maxclients; i++ ) {
		...
		if ( !Q_stricmp( ci->modelName, match->modelName ) && !Q_stricmp( ci->skinName, match->skinName ) && !Q_stricmp( ci->headModelName, match->headModelName ) && !Q_stricmp( ci->headSkinName, match->headSkinName ) && !Q_stricmp( ci->blueTeam, match->blueTeam ) && !Q_stricmp( ci->redTeam, match->redTeam ) && ( cgs.gametype < GT_TEAM || cgs.ffa_gt == 1 || ci->team == match->team ) ) {
...
static void CG_PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] ) {
	...
	if ( clientNum >= 0 && clientNum < MAX_CLIENTS ) {
		ci = &cgs.clientinfo[clientNum];
		if ( ci->fixedtorso || cg.renderingThirdPerson ) {
...
static qboolean CG_PlayerShadow( centity_t *cent, float alphaMult, float *shadowPlane, int team ) {
	...
	*shadowPlane = trace.endpos[2] + 1;
	...
	if ( cg_shadows.integer != 1 && cg_shadows.integer != 4 ) { // no mark for stencil or projection shadows
		return qtrue;
	}
