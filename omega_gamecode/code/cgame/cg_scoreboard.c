...
// Old
#define SB_OLD_INTER_HEIGHT 16
#define SB_OLD_MAXCLIENTS_INTER ( ( SB_STATUSBAR - SB_TOP ) / SB_OLD_INTER_HEIGHT - 1 )

#define SB_OLD_BOTICON_X ( SCOREBOARD_X + 32 )
#define SB_OLD_HEAD_X ( SCOREBOARD_X + 64 )

#define SB_OLD_SCORELINE_X 112

#define SB_OLD_SCORE_X ( SB_OLD_SCORELINE_X + BIGCHAR_WIDTH )         // width 6
#define SB_OLD_PING_X ( SB_OLD_SCORELINE_X + 12 * BIGCHAR_WIDTH + 8 ) // width 5
#define SB_OLD_TIME_X ( SB_OLD_SCORELINE_X + 17 * BIGCHAR_WIDTH + 8 ) // width 5
#define SB_OLD_NAME_X ( SB_OLD_SCORELINE_X + 22 * BIGCHAR_WIDTH )     // width 15
...
/*
=================
CG_DrawOldClientScore
=================
 */
static void CG_DrawOldClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	char string[1024];
	vec3_t headAngles;
	clientInfo_t *ci;
	int iconx, headx;
	...
	iconx = SB_OLD_BOTICON_X + ( SB_RATING_WIDTH / 2 );
	headx = SB_OLD_HEAD_X + ( SB_RATING_WIDTH / 2 );
	...
	if ( score->client == cg.snap->ps.clientNum ) {
		...
		hcolor[3] = fade * 0.7;
		CG_FillRect( SB_OLD_SCORELINE_X + BIGCHAR_WIDTH + ( SB_RATING_WIDTH / 2 ), y,
		             640 - SB_OLD_SCORELINE_X - BIGCHAR_WIDTH, BIGCHAR_HEIGHT + 1, hcolor );
	}

	CG_DrawBigString( SB_OLD_SCORELINE_X + ( SB_RATING_WIDTH / 2 ), y, string, fade );
...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	...
	if ( score->client == cg.snap->ps.clientNum ) {
		...
		CG_FillRect( SB_SCORELINE_X + BIGCHAR_WIDTH + ( SB_RATING_WIDTH / 2 ), y,
		             640 - SB_SCORELINE_X - BIGCHAR_WIDTH * 6, BIGCHAR_HEIGHT + 11, hcolor );
	}
...
=================
*/
static int CG_TeamScoreboard( int y, team_t team, float fade, int maxClients, int lineHeight, qboolean noDraw ) {
	int i;
	score_t *score;
	float color[4];
	int count;
	...
	for ( i = 0; i < cg.numScores && count < maxClients; i++ ) {
		...
		if ( !noDraw ) {
			if ( cg_drawOldScoreboard.integer ) {
				CG_DrawOldClientScore( y + lineHeight * count, score, color, fade, lineHeight == SB_NORMAL_HEIGHT );
			} else {
...
Draw the old in-game scoreboard
=================
 */
qboolean CG_DrawOldScoreboard( void ) {
	...
	CG_DrawPic( SB_OLD_SCORE_X + ( SB_RATING_WIDTH / 2 ), y, 64, 32, cgs.media.scoreboardScore );
	CG_DrawPic( SB_OLD_PING_X - ( SB_RATING_WIDTH / 2 ), y, 64, 32, cgs.media.scoreboardPing );
	CG_DrawPic( SB_OLD_TIME_X - ( SB_RATING_WIDTH / 2 ), y, 64, 32, cgs.media.scoreboardTime );
	CG_DrawPic( SB_OLD_NAME_X - ( SB_RATING_WIDTH / 2 ), y, 64, 32, cgs.media.scoreboardName );

	y = SB_TOP;

	// If there are more than SB_MAXCLIENTS_NORMAL, use the interleaved scores
	if ( cg.numScores > SB_MAXCLIENTS_NORMAL ) {
		maxClients = SB_OLD_MAXCLIENTS_INTER;
		lineHeight = SB_OLD_INTER_HEIGHT;
		topBorderSize = 8;
		bottomBorderSize = 16;
	} else {
	...
	if ( cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1 ) {
		...
		if ( cg.teamScores[0] >= cg.teamScores[1] ) {
			n1 = CG_TeamScoreboard( y, TEAM_RED, fade, maxClients, lineHeight, qfalse );
			CG_DrawTeamBackground( 0, y - topBorderSize, 640, n1 * lineHeight + bottomBorderSize, 0.33f, TEAM_RED );
			y += ( n1 * lineHeight ) + BIGCHAR_HEIGHT;
			maxClients -= n1;
			n2 = CG_TeamScoreboard( y, TEAM_BLUE, fade, maxClients, lineHeight, qfalse );
			CG_DrawTeamBackground( 0, y - topBorderSize, 640, n2 * lineHeight + bottomBorderSize, 0.33f, TEAM_BLUE );
			y += ( n2 * lineHeight ) + BIGCHAR_HEIGHT;
			maxClients -= n2;
		} else {
			n1 = CG_TeamScoreboard( y, TEAM_BLUE, fade, maxClients, lineHeight, qfalse );
			CG_DrawTeamBackground( 0, y - topBorderSize, 640, n1 * lineHeight + bottomBorderSize, 0.33f, TEAM_BLUE );
			y += ( n1 * lineHeight ) + BIGCHAR_HEIGHT;
			maxClients -= n1;
			n2 = CG_TeamScoreboard( y, TEAM_RED, fade, maxClients, lineHeight, qfalse );
	...
	if ( !localClient ) {
		...
		for ( i = 0; i < cg.numScores; i++ ) {
			if ( cg.scores[i].client == cg.snap->ps.clientNum ) {
				CG_DrawOldClientScore( y, &cg.scores[i], fadeColor, fade, lineHeight == SB_NORMAL_HEIGHT );
				break;
			}
		}
	}
...
/*
=================
CG_DrawNewScoreboard

Draw the new in-game scoreboard
=================
*/
qboolean CG_DrawNewScoreboard( void ) {
	...
	int maxClients;
	int lineHeight;
	int topBorderSize, bottomBorderSize, leftBorderSize, rightBorderSize;
